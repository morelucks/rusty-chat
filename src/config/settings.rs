...
impl AppConfig {
    pub fn from_env() -> Result<Self, ConfigError> {
        let app_environment = env::var("APP_ENV")
            .map_err(|e| ConfigError::NotFound(format!("APP_ENV: {}", e)))?;

        let database_url_key = match app_environment.as_str() {
            "development" => "LOCAL_DATABASE_URL",
            "production" => "PROD_DATABASE_URL",
            env => return Err(ConfigError::Message(format!(
                "Unsupported APP_ENV value '{}'. Valid values: development, production", env
            ))),
        };
